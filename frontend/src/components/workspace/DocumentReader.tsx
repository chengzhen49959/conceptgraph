...
// One case-insensitive, word-boundaried alternation over every concept name +
// alias, longest first so "self-attention" wins over "attention". Null when there
// is nothing to match — keeps the render path branchless for concept-less docs.
function buildMatcher(concepts: DocumentContent['concepts']): Matcher {
  ...
  const alts = [...byKey.keys()]
    ...
    .map(escapeRegExp)
    .join('|')
  return { regex: new RegExp(`\\b(?:${alts})\\b`, 'gi'), byKey }
}
...
export function DocumentReader({
  ...
  const rehypePlugins = useMemo(
    () => [highlightConcepts(matcher)] as unknown as MdProps['rehypePlugins'],
    [matcher],
  )
