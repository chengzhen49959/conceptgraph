...
Extract concepts and their relationships from document chunks, then keep only the concepts the document is **substantively about**: a document-level core-concept gate drops incidental mentions (prior work cited once, datasets / metrics named in passing) so the graph isn't flooded with noise.
...
Read a document where the graph was — its *original* source, faithful to how it entered the library: a **file upload** shows its parsed text, a **web clip** shows its **original page** — with the concepts it produced highlighted inline and clickable (the provenance loop run backwards: graph ⇄ source).
**Done when:** clicking a document in the sidebar opens it in place of the graph; an **uploaded file** (no `source_url`) renders its parsed source (PDF / Markdown / text → Markdown) with every mention of one of that document's concepts a clickable chip that selects the concept in the graph; a **web clip** (has `source_url`) instead **embeds its original page** (the URL it was clipped from), with a header link out for sites that refuse framing; the full graph shrinks to a corner **local graph** of that document's concepts (click a node, or the expand control, to return); a file imported before this feature still opens — its source is **backfilled from S3 on first open**.
