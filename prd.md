...
Read a document *beside* its graph — the original source faithful to how it entered the library (a **file upload** shows its parsed text, cleaned of PDF-extraction noise — figure/chart-OCR garble and stray page numbers stripped, hyphenated line-breaks rejoined — so the reading view is prose, not extractor artefacts; a **web clip** its **original page**), co-present with a synced local graph and a heading outline, so the provenance loop runs **both ways**: graph ⇄ source. Borrowed from Obsidian's reading layout (text + 关系图谱 + outline + hover preview).
**Done when:** clicking a document in the sidebar opens it **alongside** the graph in a resizable split (not replacing it); an **uploaded file** (no `source_url`) renders its parsed source (PDF / Markdown / text → Markdown — a multi-column PDF keeps its reading order, columns rendered separately so a table never scrambles the text or merges side-by-side tables) with every mention of one of that document's concepts a clickable, **hoverable** chip — clicking selects the concept in the graph, hovering peeks its description + passages (CJK names link inline too); the right pane is the graph **scoped to this document's concepts**; the link is **bidirectional** — clicking a node scrolls the prose to that concept and pulses it, scrolling the prose highlights the concept's node; a heading **outline** rail navigates the document and tracks the current section; a **web clip** (has `source_url`) instead **embeds its original page**, with a header link out for sites that refuse framing (no inline links, so no graph pane); a file imported before this feature still opens — its source is **backfilled from S3 on first open**.
