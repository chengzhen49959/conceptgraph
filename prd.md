...
> Scope note: the single-user core loop is F1–F10 (plus F16, the web-clip ingestion client); the collaboration layer is F11–F15; F18 exposes the core loop to external AI clients and **F19 brings that agent in-product** (built — conversation + multi-step tool loop + graph-aware concept-first + hybrid lexical/vector retrieval). F15 (Slack) is the sole remaining unbuilt feature; F7 (semantic search) and F8 (RAG Q&A) are built.
