...
Upload PDF / Markdown / text, triggering an async processing pipeline. Images are rejected at upload (415) — there is no OCR, so they carry no extractable text.
**Done when:** upload returns immediately and shows progress — the documents list shows the **live ingest stage** (parsing → embedding → extracting → merging → clustering → ready) while it processes, not just a spinner; once the pipeline finishes, the document's concepts appear on the graph and status flips to done; large files (e.g., a 20MB PDF) do not block or time out; uploading an image returns a clear "not supported" error instead of silently producing a garbage node.

### F3 Automatic Concept Extraction **[built]**
Extract concepts and their relationships from document chunks, then keep only the concepts the document is **substantively about**: a document-level core-concept gate drops incidental mentions (prior work cited once, datasets / metrics named in passing) so the graph isn't flooded with noise. The gate judges centrality against the document's own **framing** (its title + abstract + introduction + conclusion) rather than an aggregate summary of every passage — so a tool the document merely *uses* (an optimizer, a normalization trick) is not mistaken for a topic the document is *about*.
**Done when:** after a document is processed, its core concept nodes and relationship edges appear on the graph, each concept traces back to its source chunk, and one-off background mentions (e.g. a paper cited once in related work) do **not** become nodes.
...
### F17 Source Reading View **[built]**
Read a document *beside* its graph — the original source faithful to how it entered the library (a **file upload** shows its parsed text, a **web clip** its **original page**), co-present with a synced local graph and a heading outline, so the provenance loop runs **both ways**: graph ⇄ source. Borrowed from Obsidian's reading layout (text + 关系图谱 + outline + hover preview).
**Done when:** clicking a document in the sidebar opens it **alongside** the graph in a resizable split (not replacing it); an **uploaded file** (no `source_url`) renders its parsed source (PDF / Markdown / text → Markdown) with every mention of one of that document's concepts a clickable, **hoverable** chip — clicking selects the concept in the graph, hovering peeks its description + passages (CJK names link inline too); the right pane is the graph **scoped to this document's concepts**; the link is **bidirectional** — clicking a node scrolls the prose to that concept and pulses it, scrolling the prose highlights the concept's node; a heading **outline** rail navigates the document and tracks the current section; a **web clip** (has `source_url`) instead **embeds its original page**, with a header link out for sites that refuse framing (no inline links, so no graph pane); a file imported before this feature still opens — its source is **backfilled from S3 on first open**.
